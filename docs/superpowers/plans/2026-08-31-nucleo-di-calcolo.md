...
4. **Le celle costruite contigue alla stessa altezza formano un Edificio.** Il campo che numera gli edifici nel `.INX` spezza tre blocchi in sette identificativi, alcuni di una cella sola, quindi non è utilizzabile — ma raggruppare per **sola altezza** fonde blocchi separati che stanno alla stessa quota. Serve la contiguità, come dice `docs/formato-inx.md`: un flood fill a quattro vicini fra celle della stessa altezza. Sul caso reale dà tre componenti — due da 91 celle a 6,0 m e una da 98 a 3,0 m — che coincidono con i due duplex e il simplex della relazione. *Una versione precedente di questo punto diceva "per altezza" e sbagliava; è stato scoperto contando.*
...
10. **Le Superfici vengono da entrambe le fonti.** Il caso reale ha `soil_profiles` a `000000` in tutte 2500 le celle, ma `plants_2d` porta un codice di erba in **1602 celle su 2500**: leggere solo i profili di suolo dichiara terreno nudo dove i due terzi del dominio sono erba.
11. **Le impronte si uniscono in rettangoli massimali** prima di essere scritte: run-length lungo un asse, poi coalescenza sull'altro. Sul caso reale porta 2780 rettangoli a 4, e un file di Scenario da 374 KB a pochi. L'ordine conta: costruire le maschere, togliere le sovrapposizioni, unire **alla fine**.
